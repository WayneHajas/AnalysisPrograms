...
class NewMDB:
    ...
    def ADDTo_TranChar(self, Location,SiteNum,Year,StatArea,SubArea,InBed,NumTran,\
                       TransectArea):
        query ="insert INTO Results_TranChar(AnalysisKey,TranCharKey,Location,SiteNum,Yr,StatArea,SubArea,InBed,"
        ...
        try:
            ...
            print('\nNewMDB 279 query\n',query)
            self.DB.Execute(query)
        self.TranCharMap+=[[Location,SiteNum,Year,StatArea,SubArea,InBed,self.TranCharKey.GetValue() ]] 
        self.TranCharKey.Increment()
   


    def GetTranCharKey(self,Location,SiteNum,Year,StatArea,SubArea,InBed):
        ...
        try:
            x=list(filter(lambda t:self.TranCharMap[t][:6]==[Location,SiteNum,Year,StatArea,SubArea,InBed],range(len(self.TranCharMap))))[0]
            result= self.TranCharMap[x][-1]
            return(result)
        except:
            print('\nNewMDB 310')
            print('[Location,SubSampleLocation,Yr,StatArea,SubArea,InBed]',[Location,SiteNum,Year,StatArea,SubArea,InBed])
