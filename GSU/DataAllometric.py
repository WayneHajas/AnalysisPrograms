...
class DataAllometric:
    ...
    def BuildAlloEqun(self):
        ...
        if (self.TD==[]) or (self.TD==None) or (self.W==[]) or (self.W==None) :
            ...
            return
        x=list(map(lambda t:log(t/30.),self.TD))
        ...
        sigmaW=std(eps,ddof=1)

        #page 34
        SDbeta=sigmaW/sqrt(Sxx)
        sumXsqr=sum(list(map(lambda t:t*t,x)))
        SDLnW30=sigmaW*sqrt(sumXsqr/float(n)/Sxx)
        self.Allo=AlloEqn(b0,SDLnW30,b1,SDbeta,sigmaW)
